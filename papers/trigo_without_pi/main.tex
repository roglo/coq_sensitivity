...
What about multiplication of two angles? It seems not
possible. Because {\em addition} of angles is actually {\em
  multiplication} of complex numbers, when we consider angle $(x,y)$
as the complex number $x+iy$: indeed if we apply rules of
multiplications in complex numbers, we get exactly as result the rules
for additions of angles. Therefore, perhaps, multiplication of angles
could be something that {\em exponentiation} of complex numbers? But
this intuition seems not to work.

Moreover, multiplication of angles has no meaning, as far as geometry
is concerned. In this work, we did not define multiplication of
angles.

This has a consequence: it is not possible to define exponentiation of
imaginary angles, like in the famous expression $e^{i\theta}$ of
classical trigonometry, supposed to be equal to
...
i.e. its parameter $x$ {\em multiplied} several times by
itself. Euler's formula therefore doesn't work, but we see further,
that Moivre's formula works in our trigonometry.

\

\noindent But what we need is division by a natural. We defined above
multiplication by a natural. Division is more complicated but we see
in next sections that it is possible to define it.
