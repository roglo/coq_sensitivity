...
erewrite bmat_el_BM_M; [ | apply Hsizes' | | | ]; cycle 1. {
  rewrite <- HAB'.
  injection HAB; clear HAB; intros HAB; rewrite <- HAB.
  cbn - [ iter_seq ].
...
cbn - [ iter_seq ].
injection HAB; clear HAB; intros HAB.
...
rewrite sizes_of_bmatrix_fold_left in Habs. 2: {
  ...
  rewrite sizes_of_bmatrix_add. 2: {
    ...
    symmetry.
    apply sizes_of_bmatrix_mul; [ | | congruence ]. {
      unfold is_square_bmat; apply Ha; flia Hk.
    } {
      unfold is_square_bmat; apply Hb; flia Hk.
    }
  }
  rewrite sizes_of_bmat_zero_like.
  symmetry.
...
rewrite sizes_of_bmatrix_add in Habs. 2: {
  ...
  symmetry.
  apply sizes_of_bmatrix_mul; [ | | congruence ]. {
    unfold is_square_bmat; apply Ha; flia Hzrb Hrbs.
  } {
    unfold is_square_bmat; apply Hb; flia Hzrb Hrbs.
  }
}
...
rewrite IHAB with (len := size * len') (size := size) (sizes := sizes) (A := BM_M MA) (B := BM_M MB);
    cycle 1. {
  rewrite HAB; cbn.
