...
...
  ...
  destruct lb as [| c]. {
    ...
    remember (rel a b) as ab eqn:Hab; symmetry in Hab.
    destruct ab; [ | now f_equal; destruct it ].
    specialize (Hant a b Hab Hba) as H; subst b.
    f_equal; clear Hab; rename Hba into Haa.
    apply split_nil_r in Hl'.
    destruct Hl' as (_, Hla).
    destruct la as [| b]; [ | cbn in Hla; flia Hla ].
    now destruct it.
  }
  apply split_cons_cons in Hl'.
  destruct Hl' as (l' & Hl' & H); subst l.
  rename l' into l.
...
...
    ...
    cbn in H1.
    destruct la as [| d]. {
      injection H1; clear H1; intros; subst l.
      apply split_nil_l in Hl'.
      destruct Hl' as (Hl', _); subst lb.
      destruct it; [ | easy ].
      cbn in Hll; flia Hll.
    }
    remember (rel d c) as dc eqn:Hdc; symmetry in Hdc.
...
...
    ...
    apply IHit; [ | | | easy ]. 3: {
      destruct l as [| b]. {
        cbn in Hl'.
        injection Hl'; clear Hl'; intros; subst la lb.
        destruct it; [ easy | ].
        cbn in H1 |-*.
...
...
...
  ...
  destruct la as [| c]. {
    f_equal.
    apply split_nil_l in Hl'.
    now destruct Hl'; subst l lb.
  }
  remember (rel c b) as cb eqn:Hcb; symmetry in Hcb.
  destruct cb. 2: {
    f_equal.
    apply IHit; [ now transitivity (S it) | easy | | easy ].
    now do 2 apply sorted_cons in Hs.
  }
  destruct lb as [| d]. {
    apply split_nil_r in Hl'.
    destruct Hl' as (H, Hlen); subst l.
    destruct la; [ | cbn in Hlen; flia Hlen ].
    cbn in Hs.
    rewrite Hab in Hs.
