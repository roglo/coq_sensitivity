...
Theorem lemma_2_A_n_2_eq_n_I :
  rngl_has_opp = true →
  ∀ n, (mA n * mA n)%M = (rngl_mul_nat_l n 1 × mI (2 ^ n))%M.
Proof.
intros Hop *.
...
Theorem μ_is_ev_of_An_iff_μ2_eq_n :
  ...
  ∀ n μ,
  (∃ V, is_eigenvector_of_An n μ V) ↔ (μ * μ = rngl_mul_nat_l n 1)%F.
Proof.
