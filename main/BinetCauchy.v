...
subst a.
specialize (sls1n_bounds n (S k) (S n :: t)) as H1.
rewrite <- Ht in H1.
remember (sls1n n (S k)) as ll eqn:Hll.
assert (H : nth j ll [] ∈ ll). {
  apply nth_In; rewrite Hll, sls1n_length.
  flia Hi Hik Hj.
}
specialize (H1 H (S n)); clear H.
rewrite Ht in H1.
...
Theorem sub_lists_of_seq_1_n_1_r : ∀ n,
  sub_lists_of_seq_1_n n 1 = map (λ i, [i]) (seq 0 n).
Proof.
intros.
induction n; [ easy | ].
rewrite seq_S; cbn.
