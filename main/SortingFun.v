...
(*
Compute (let la := [0;2;1] in
apply_transp_list (transp_list Nat.eqb (seq 0 (length la)) la) la).
Definition succ_when_ge k a := a + Nat.b2n (k <=? a).
Fixpoint canon_sym_gr_list n k : list nat :=
  match n with
  | 0 => []
...
Compute (map (λ la,
(la,
apply_transp_list (transp_list Nat.eqb (seq 0 (length la)) la) (seq 0 (length la))))
 (canon_sym_gr_list_list 4)).
Compute (map (λ la,
(
apply_transp_list (transp_list Nat.eqb la (seq 0 (length la))) la))
 (canon_sym_gr_list_list 4)).
*)
...
rewrite List_cons_is_app in Hpab.
...
revert lb.
induction la as [| a]; intros; cbn. {
  destruct lb as [| b]; [ apply perm_nil | ].
  apply Permutation_refl.
}
destruct lb as [| b]. {
  rewrite app_nil_r.
...
apply perm_skip.
apply Permutation_trans with (l' := (b :: (la ++ lb))). {
  rewrite List_app_cons, app_assoc, app_comm_cons.
...
move b before a.
move lb before la; move lc before lb; move ld before lc.
specialize (Permutation_in a Hac (or_introl eq_refl)) as Hc.
specialize (Permutation_in b Hbd (or_introl eq_refl)) as Hd.
apply in_split in Hc, Hd.
destruct Hc as (lc1 & lc2 & Hc).
destruct Hd as (ld1 & ld2 & Hd).
subst lc ld.
