...
Theorem canon_sym_gr_elem_inj1 : ∀ n k i j,
  ...
  → i < n
  → j < n
  → canon_sym_gr_elem n k i = canon_sym_gr_elem n k j
  → i = j.
Proof.
intros * Hk Hi Hj Hij.
cbn in Hij.
...
Theorem canon_sym_gr_elem_inj2 : ∀ n i j,
  i < n!
  → j < n!
  → (∀ k, k < n → canon_sym_gr_elem n i k = canon_sym_gr_elem n j k)
  → i = j.
Proof.
intros * Hin Hjn Hij.
...
split. {
  ...
  apply (canon_sym_gr_elem_inj2 Hi Hj).
  intros k Hk.
  apply H1.
  apply in_seq.
  split; [ flia | easy ].
  ...
  split. {
    ...
    assert
      (Hr :
       rank_of_permut_in_canon_sym_gr n
         (sub_canon_permut_fun (vect_el 0 (mk_vect l))) < n!). {
      apply rank_of_canon_permut_ub.
      unfold sub_canon_permut_fun.
      cbn - [ "<?" ].
    ...
    destruct (le_dec _ _) as [H1| H1]. {
      ...
      remember
        (rank_of_permut_in_canon_sym_gr n
           (sub_canon_permut_fun (vect_el 0 (mk_vect l)))) as k eqn:Hk.
Search sub_canon_permut_fun.
Print sub_canon_permut_fun.
...
      apply rank_of_canon_permut_ub.
      unfold sub_canon_permut_fun.
...
