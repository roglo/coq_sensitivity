...
destruct n. {
  ...
  destruct (Nat.eq_dec (vect_size vv) 0) as [Hvz| Hvz]. {
    now rewrite Hvz in Hil.
  }
  destruct (Nat.eq_dec (vect_size vv) 1) as [Hv1| Hv1]; [ easy | ].
  specialize (Hsg 0) as H1.
  specialize (Hsg 1) as H2.
  specialize (Hinj 0 1) as H3.
  assert (H : 0 < vect_size vv) by flia Hvz.
  specialize (H1 H); specialize (H3 H); clear H.
  assert (H : 1 < vect_size vv) by flia Hvz Hv1.
  specialize (H2 H); specialize (H3 H); clear H.
  destruct H1 as (H4, H5).
  destruct H2 as (H6, H7).
  enough (H : vect_el empty_vect vv 0 = vect_el empty_vect vv 1). {
    now apply H3 in H.
  }
  unfold vect_size in H4, H6.
  apply length_zero_iff_nil in H4, H6.
  unfold vect_el in H4, H6 |-*.
  remember (nth 0 (vect_list vv) empty_vect) as x eqn:Hx.
  remember (nth 1 (vect_list vv) empty_vect) as y eqn:Hy.
  symmetry in Hx, Hy.
  destruct x as (x).
  destruct y as (y).
  cbn in H4, H6.
...
assert (H : ∀ s, s < S (S n) → is_sym_gr_vect (S n) (vv' s)). {
  ...
  split. {
    ...
    split. {
      ...
      rewrite Hk in H6.
      specialize (H6 Hjn).
      destruct (Nat.eq_dec (nth j la 0) (S n)) as [Hjn'| Hjn']. {
        symmetry in Hjn'; specialize (H6 Hjn'); clear Hjn' Hjn; subst s.
        ...
        destruct (Nat.eq_dec j (S n)) as [Hjn| Hjn]. {
          subst j.
          rewrite nth_overflow; [ flia | now rewrite Hl ].
        }
        specialize (H5 j (S j) Hs) as H7.
        assert (H : S j < S (S n)) by flia Hs Hjn.
        specialize (H6 H); specialize (H7 H); clear H.
        rewrite Hk in H7.
        destruct (Nat.eq_dec (nth (S j) la 0) (S n)) as [H| H]. {
          symmetry in H; specialize (H7 H); clear H; flia H7.
        }
        flia H6 H.
      } {
        ...
        destruct (Nat.lt_trichotomy j s) as [Hjs| [Hjs| Hjs]]. {
          ...
          specialize (H4 (S j)) as H7.
          specialize (H5 s (S j) Hs) as H8.
          assert (H : S j < S (S n)) by flia Hjn Hjsn.
