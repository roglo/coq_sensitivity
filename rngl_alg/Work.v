...
Theorem seq_angle_to_div_nat_7_le :
  ∀ i θ, (seq_angle_to_div_nat θ 7 i ≤ 5 * (θ / ₂^5))%A.
Proof.
intros.
(* 1/7 = 1/8 + 1/64 + ...
   1/7 < 1/8 + 1/32 = 5/32 *)
...
assert (Hm5 : S (S m) * 2 ^ 2 ≤ 2 ^ 5). {
  ...
  do 6 apply Nat.succ_lt_mono in Hm.
  apply Nat.lt_1_r in Hm; subst m; cbn.
  now do 28 apply -> Nat.succ_le_mono.
}
destruct (lt_dec i 5) as [Hi5| Hi5]. {
  ...
  do 3 apply Nat.succ_lt_mono in Hi5.
  destruct i. {
    clear Hni Hi5.
    rewrite (Nat_div_less_small 1); [ | cbn; flia ].
    rewrite Nat.mul_1_r.
    ...
    rewrite angle_mul_nat_assoc.
    apply angle_mul_le_mono_r. 2: {
      do 6 (destruct m; [ now do 5 apply -> Nat.succ_le_mono | ]).
      now do 7 apply Nat.succ_lt_mono in Hm.
    }
    apply (angle_mul_nat_not_overflow_le_l _ (2 ^ 5)); [ easy | ].
    now apply angle_mul_nat_overflow_div_2_pow.
  }
...
apply angle_mul_le_mono_r. {
  ...
  apply Nat.div_le_upper_bound; [ easy | ].
  apply Nat.mul_le_mono_r.
  destruct m; [ now do 2 apply -> Nat.succ_le_mono | ].
  destruct m; [ now do 3 apply -> Nat.succ_le_mono | ].
  destruct m; [ now do 4 apply -> Nat.succ_le_mono | ].
  destruct m; [ now do 5 apply -> Nat.succ_le_mono | ].
  destruct m; [ now do 6 apply -> Nat.succ_le_mono | ].
  do 6 apply Nat.succ_lt_mono in Hm.
  now apply Nat.lt_1_r in Hm; subst m.
}
clear Hni Hθ'.
induction i; [ easy | ].
  ...
  destruct m. {
    ...
    destruct (lt_dec i 6) as [Hi6| Hi6]. {
      ...
      destruct i. {
        ...
        replace (4 * 2 ^ 3) with (2 ^ 5) by easy.
        replace (3 + 3) with 6 by easy.
        rewrite angle_div_2_pow_succ_r_2.
        now apply angle_mul_nat_overflow_div_2_pow.
      }
      destruct i. {
        ...
        replace (4 * 3 * 2 ^ 2) with (3 * 2 ^ 4) by easy.
        apply (angle_mul_nat_not_overflow_le_l _ (2 ^ 6)). {
          now cbn; do 48 apply -> Nat.succ_le_mono.
        }
        now apply angle_mul_nat_overflow_div_2_pow.
      }
      destruct i. {
        ...
        cbn - [ angle_div_2_pow angle_mul_nat_overflow ].
        replace 48 with (3 * 2 ^ 4) by easy.
        apply (angle_mul_nat_not_overflow_le_l _ (2 ^ 6)). {
          now cbn; do 48 apply -> Nat.succ_le_mono.
        }
        now apply angle_mul_nat_overflow_div_2_pow.
...
destruct n. {
  ...
  destruct m. {
    ...
    rewrite Nat.mul_assoc.
    eapply Nat.le_trans. {
      apply (Nat.mul_le_mono_r _ 6); cbn.
      now do 4 apply -> Nat.succ_le_mono.
    }
    now apply Nat.mul_div_le.
  }
  ...
  destruct m. {
    ...
    destruct (lt_dec i 4) as [Hi4| Hi4]. {
      ...
      destruct i. {
        ...
        replace (4 * 2 ^ 1) with (2 ^ 3) by easy.
...
destruct n. {
  ...
  destruct m. {
    ...
    rewrite Nat.mul_assoc.
    eapply Nat.le_trans. {
      apply (Nat.mul_le_mono_r _ 7); cbn.
      now do 4 apply -> Nat.succ_le_mono.
    }
    now apply Nat.mul_div_le.
  }
  ...
  apply Nat.succ_lt_mono in Hm.
  destruct m. {
    apply (angle_add_overflow_mul_by_lt_7 i θ); [ easy | easy | ].
