...
Theorem rngl_angle_opt_le_dec :
  let roa := angle_ring_like_op in
  if rngl_is_ordered (angle T) then
    ∀ a b : angle T, {(a ≤ b)%L} + {¬ (a ≤ b)%L}
  else not_applicable.
Proof. intros; cbn; apply angle_le_dec. Qed.

Theorem rngl_angle_opt_le_refl :
  let roa := angle_ring_like_op in
  if rngl_is_ordered (angle T) then ∀ a : angle T, (a ≤ a)%L
  else not_applicable.
Proof. intros; cbn; apply angle_le_refl. Qed.

Theorem rngl_angle_opt_le_antisymm :
  let roa := angle_ring_like_op in
  if rngl_is_ordered (angle T) then
    ∀ a b : angle T, (a ≤ b)%L → (b ≤ a)%L → a = b
  else not_applicable.
Proof. cbn; apply angle_le_antisymm. Qed.

Theorem rngl_angle_opt_le_trans :
  let roa := angle_ring_like_op in
  if rngl_is_ordered (angle T) then
    ∀ a b c : angle T, (a ≤ b)%L → (b ≤ c)%L → (a ≤ c)%L
  else not_applicable.
Proof. cbn; apply angle_le_trans. Qed.

Theorem rngl_angle_opt_add_le_compat :
  let roa := angle_ring_like_op in
  if rngl_is_ordered (angle T) then
    ∀ a b c d : angle T, (a ≤ b)%L → (c ≤ d)%L → (a + c ≤ b + d)%L
  else not_applicable.
Proof.
cbn; intros.
apply (angle_le_trans _ (b + c)). {
  apply angle_add_le_mono_r; [ | easy ].
(* ah ouais, ça déconne, là ; il faut que b + c ne déborde pas
  ============================
  angle_add_overflow b c = false
*)
...

Theorem rngl_angle_opt_mul_le_compat_nonneg : True. easy. Qed.
Theorem rngl_angle_opt_mul_le_compat_nonpos : True. easy. Qed.
Theorem rngl_angle_opt_not_le : True. easy. Qed.
Theorem rngl_angle_opt_archimedean : True. easy. Qed.
...
Definition angle_ring_like_prop :
  ...
  {| rngl_mul_is_comm := true;
     ...
     rngl_opt_le_dec := rngl_angle_opt_le_dec;
     rngl_opt_integral := NA;
     rngl_opt_alg_closed := NA;
     rngl_opt_characteristic_prop := NA;
     rngl_opt_le_refl := rngl_angle_opt_le_refl;
     rngl_opt_le_antisymm := rngl_angle_opt_le_antisymm;
     rngl_opt_le_trans := rngl_angle_opt_le_trans;
     rngl_opt_add_le_compat := rngl_angle_opt_add_le_compat;
     rngl_opt_mul_le_compat_nonneg := rngl_angle_opt_mul_le_compat_nonneg;
     rngl_opt_mul_le_compat_nonpos := rngl_angle_opt_mul_le_compat_nonpos;
     rngl_opt_mul_le_compat_non_opp := NA;
     rngl_opt_not_le := rngl_angle_opt_not_le;
     rngl_opt_archimedean := rngl_angle_opt_archimedean |}.
*)
