...
destruct (Nat.eq_dec (rngl_characteristic T) 1) as [Hc1| Hc1]. {
  intros.
  specialize (rngl_characteristic_1 Hon Hos Hc1) as H1.
  rewrite (characteristic_0_angle_0 Hon Hos Hc1 (θ / ₂)%A).
  apply (angle_add_overflow_0_0 Hon Hos).
}
...
destruct zs. {
  ...
  apply (rl_sqrt_lt_rl_sqrt Hic Hop Hor). {
    apply (rngl_div_pos Hon Hop Hiv Hor). 2: {
      apply (rngl_0_lt_2 Hon Hop Hc1 Hor).
    }
    apply (rngl_le_opp_l Hop Hor).
...
(* oh, tu sais, je commence à en avoir plein le cul, là *)
...
    apply rngl_add_lt_compat_nonneg.
    apply (rngl_le_opp_l Hop Hor).
    apply (rngl_cos_bound Hon Hop Hiv Hic Hed Hor).
...
...
  ...
  apply Nat.log2_lt_cancel in Hi.
...
rewrite <- angle_div_2_pow_nat_div_2_succ.
...
...
...
    ...
    specialize (H1 (angle_div_2_pow_nat θ i) (θ))%A.
    apply H1.
...
...
split; [ | now apply (IHn _ θ2) ].
...
  eapply (angle_add_overflow_le Hic Hon Hop Hed _ (S n * θ2))%A. 2: {
...
split. {
  eapply (angle_add_overflow_le Hic Hon Hop Hed _ (S n * θ2))%A.
... ...
