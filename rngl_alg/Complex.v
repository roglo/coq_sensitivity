...
apply rngl_cos_cos_sin_sin_nonneg_sin_le_cos_le_iff; try easy.
apply (rngl_cos_sub_nonneg Hop); try easy.
...
progress unfold angle_ltb.
generalize Hs1z; intros H.
apply (rngl_sin_add_nonneg Hop θ3) in H; try easy.
...
destruct (rngl_le_dec Hor 0 (rngl_cos θ1)) as [Hzc1| Hc1z]. {
  ...
  subst θ1; rename θ into θ1.
  rewrite <- (angle_add_sub_swap Hic Hop) in Hzs13 |-*.
  rewrite (rngl_sin_sub_straight_r Hon Hop) in Hzs13, Hs1z.
  rewrite (rngl_cos_sub_straight_r Hon Hop) in Hc1z |-*.
  apply (rngl_opp_nonneg_nonpos Hop Hor) in Hzs13.
  apply (rngl_opp_nonneg_nonpos Hop Hor).
  apply (rngl_opp_nonpos_nonneg Hop Hor) in Hs1z.
...
destruct (rngl_eq_dec Hed (rngl_sin θ1) (rngl_sin θ2)) as [Hs12| Hs12]. {
  apply (rngl_sin_eq Hic Hon Hop Hed) in Hs12.
  ...
  symmetry in Hzc1.
  rewrite Hzc1.
  apply (eq_rngl_cos_0 Hic Hon Hop Hed) in Hzc1.
...
Ltac change_angle_sub_right Hic Hon Hop Hed Hor θ :=
  remember (θ - angle_right)%A as θ' eqn:Hθ';
  apply (angle_add_move_r Hic Hon Hop Hed) in Hθ';
  subst θ; rename θ' into θ;
  (try rewrite (angle_add_add_swap Hic Hop) in *).

Ltac sin_cos_add_sub_right_hyp Hon Hop Hor H :=
  assert (Hos' : rngl_has_opp_or_subt T = true) by
    apply (rngl_has_opp_has_opp_or_subt Hop);
  try rewrite (rngl_sin_add_right_r Hon Hos') in H;
  try rewrite (rngl_cos_add_right_r Hon Hop) in H;
  try rewrite (rngl_sin_sub_right_r Hon Hos') in H;
  try rewrite (rngl_cos_sub_right_r Hon Hop) in H;
...
Ltac sin_cos_add_sub_right Hon Hop Hor :=
  assert (Hos' : rngl_has_opp_or_subt T = true) by
    apply (rngl_has_opp_has_opp_or_subt Hop);
  try rewrite (rngl_sin_add_right_r Hon Hos');
...
rewrite (angle_add_sub_assoc Hop) in Hzs13.
rewrite (rngl_sin_sub_straight_r Hon Hop) in Hzs13, Hzs3.
...
apply (rngl_nlt_ge Hor) in Hc1z.
(**)
change_angle_sub_right Hic Hon Hop Hed Hor θ1.
sin_cos_add_sub_right_hyp Hon Hop Hor Hzs13.
sin_cos_add_sub_right_hyp Hon Hop Hor Hzs1.
sin_cos_add_sub_right_hyp Hon Hop Hor Hc1z.
sin_cos_add_sub_right Hon Hop Hor.
(*
remember (θ1 - angle_right)%A as θ.
apply (angle_add_move_r Hic Hon Hop Hed) in Heqθ.
subst θ1; rename θ into θ1.
    ...
    destruct zs3. {
      ...
      destruct zs13. {
        ...
        destruct (rngl_le_dec Hor 0 (rngl_cos θ3)) as [Hzc3| Hc3z]. {
          ...
          rewrite (rngl_sin_sub_right_r Hon Hop) in Hzs3, Hzs13.
          rewrite (rngl_cos_sub_right_r Hon Hop) in Hzc3 |-*.
          apply (rngl_opp_neg_pos Hop Hor) in Hzs3, Hzs13.
          destruct (rngl_le_dec Hor (rngl_cos θ2) 0)%L as [Hc2z| Hzc2]. {
            remember (θ2 - angle_right)%A as θ eqn:Hθ.
            apply (angle_add_move_r Hic Hon Hop Hed) in Hθ.
            subst θ2; rename θ into θ2; move θ2 before θ3.
...
(**)
              change_angle_sub_right Hic Hon Hop Hed Hor θ1.
              sin_cos_add_sub_right_hyp Hon Hop Hor Hzs12.
              sin_cos_add_sub_right_hyp Hon Hop Hor Hc1z.
              sin_cos_add_sub_right_hyp Hon Hop Hor Hzs1.
              sin_cos_add_sub_right Hon Hop Hor.
(*
...
*)
            ...
            destruct (rngl_le_dec Hor 0 (rngl_cos θ1)) as [Hzc1| Hc1z]. {
              ...
              rewrite (rngl_cos_sub_right_r Hon Hop) in Hzs12, Hzs13, Hzc1.
              apply (rngl_opp_neg_pos Hop Hor) in Hs1z.
              exfalso.
              apply Bool.not_true_iff_false in Haov13.
              apply Haov13; clear Haov13.
            ...
            rewrite (rngl_sin_sub_straight_r Hon Hop) in Hs1z.
            do 2 rewrite (rngl_sin_sub_straight_r Hon Hop).
            rewrite (rngl_cos_sub_straight_r Hon Hop) in Hzs12, Hzs13, Hc1z.
            apply (rngl_opp_neg_pos Hop Hor) in Hzs12, Hc1z, Hs1z.
