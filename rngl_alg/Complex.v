...
symmetry in Hnt.
destruct nt. 2: {
  now apply (angle_div_nat_is_inf_sum_of_angle_div_2_pow Har Hch).
}
...
specialize angle_div_nat_is_inf_sum_of_angle_div_2_pow as H1.
...
eapply (angle_lim_eq_compat 0 0) in H1. 2: {
  intros i.
  rewrite Nat.add_0_r.
  rewrite angle_div_2_pow_mul; [ | easy | easy ].
  rewrite (angle_mul_nat_assoc Hon Hop).
  easy.
}
...
eapply (angle_lim_eq_compat 0 0) in H1. 2: {
  ...
  rewrite Nat.mul_comm.
  rewrite <- (angle_mul_nat_assoc Hon Hop).
  rewrite <- angle_div_2_pow_mul; cycle 1. {
    now apply Nat.pow_nonzero.
  } {
    rewrite Hθ''.
    apply angle_mul_nat_overflow_pow_div.
...
eapply (angle_lim_eq_compat 0 0) in H1. 2: {
  ...
  rewrite Nat.mul_comm.
  rewrite <- (angle_mul_nat_assoc Hon Hop).
  rewrite angle_div_2_pow_mul; [ | easy | now rewrite <- Hθ'' ].
...
...
    apply rngl_cos_lt_rngl_cos_sub; try easy. {
      ...
      apply eq_angle_div_2_pow_0 in H1.
...
rewrite angle_div_2_pow_add.
...
rewrite <- (angle_div_2_pow_mul _ _ θ').
3: {
(* ouais, bon, n'importe quoi *)
...
Search (_ * (_ / ₂^_))%A.
rewrite <- (angle_div_2_pow_mul i (2 ^ i / n) θ').
3: {
...
rewrite <- angle_div_2_pow_mul in HN.
...
assert (Hε2 : (0 < ε / 2)%L). {
  apply (rngl_lt_div_r Hon Hop Hiv Hor).
  apply (rngl_0_lt_2 Hon Hop Hc1 Hor).
...
...
destruct n; [ easy | ].
apply angle_mul_nat_overflow_succ_l_true in Hnt.
destruct Hnt as [Hnt| Hnt]. {
  destruct n; [ easy | ].
  apply angle_mul_nat_overflow_succ_l_true in Hnt.
...
  rewrite angle_div_2_pow_mul; cycle 2. {
...
...
  rewrite Hθ''.
  rewrite angle_div_2_pow_mul; [ | easy | ]. 2: {
    now rewrite <- Hθ''.
  }
  rewrite <- angle_div_pow_2_add_distr.
...
apply (angle_lim_eq_compat 0 j (λ i, 2 ^ (i + j) / n * n * (θ' / ₂^j) / ₂^i)%A). {
  ...
  rewrite angle_div_pow_2_add_distr.
(*
  rewrite angle_div_2_pow_mul; [ | admit | ]. 2: {
Search (angle_mul_nat_overflow _ (_ / ₂^_)).
...
*)
...
  rewrite angle_div_2_pow_mul; [ | admit | admit ].
  easy.
}
...
eapply (angle_lim_eq_compat 0 j). {
...
apply (angle_lim_eq_compat j 0) with
  ...
  intros i.
  rewrite Nat.add_0_r.
  rewrite (angle_div_2_pow_mul _ _ _ Hnz Htj).
  rewrite (angle_mul_nat_assoc Hon Hop).
  rewrite Nat.mul_comm.
...
Search (_ * (_ / ₂^_))%A.
rewrite <- angle_div_2_pow_mul in HN; cycle 1. {
  intros H.
...
...
  revert θ.
  induction n; intros; [ easy | ].
Check angle_mul_nat_overflow_succ_l_true.
...
  apply (angle_mul_nat_overflow_succ_l_true Hon Hos).
...
(*
...
apply (angle_mul_nat_overflow_succ_l_false Hon Hos) in Hab.
apply (angle_mul_nat_overflow_succ_l_false Hon Hos).
...
specialize (Hu _ Hε2) as H1.
specialize (Hv _ Hε2) as H2.
...
intros H2.
...
specialize (angle_eucl_dist_triangular (v N) (u N) θ) as H4.
specialize (angle_eucl_dist_triangular θ' (v N) (u N)) as H5.
...
progress unfold ε in H1.
...
rewrite <- (rngl_add_diag Hon).
eapply (rngl_le_trans Hor). {
  apply (angle_eucl_dist_triangular _ (u N)).
}
rewrite (angle_eucl_dist_symmetry Hic Hop).
apply (rngl_add_le_mono_l Hop Hor).
apply angle_dist_le_l; [ easy | ].
...
rewrite <- (rngl_add_diag Hon).
eapply (rngl_le_trans Hor). {
  apply (angle_eucl_dist_triangular _ (v N)).
}
apply (rngl_add_le_mono_r Hop Hor).
rewrite (angle_eucl_dist_symmetry Hic Hop (v N)).
...
remember (rngl_abs _) as x eqn:Hx in H1.
rewrite <- (rngl_abs_nonneg_eq Hop Hor √_%L) in H1. 2: {
  apply rl_sqrt_nonneg.
  apply (rngl_mul_nonneg_nonneg Hop Hor). {
    apply (rngl_0_le_2 Hon Hop Hor).
  }
