...
destruct_ac.
intros.
remember (angle_mul_nat_overflow (_ * _) _) as b eqn:Hb.
symmetry in Hb.
destruct b. {
  revert n θ Hb.
  induction m; intros; [ easy | ].
...
symmetry.
remember (angle_mul_nat_overflow (S m * n) θ) as c eqn:Hc.
symmetry in Hc |-*.
destruct c. {
...
...
Search (_ || _ = _)%bool.
...
remember (angle_mul_nat_overflow (S _ * _) _) as x eqn:Hx.
symmetry
rewrite angle_mul_
...
