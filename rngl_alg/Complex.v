...
Theorem rngl_cos_add_le_cos_if :
  rngl_mul_is_comm T = true →
  rngl_has_1 T = true →
  rngl_has_opp T = true →
  rngl_has_eq_dec T = true →
...
(**)
      progress unfold angle_add_overflow in Haov.
      progress unfold angle_add_overflow in Haov'.
      apply angle_ltb_ge in Haov, Haov'.
      ...
      rewrite Hzs1, Hzs12 in Haov'.
      apply rngl_leb_le in Haov, Haov'.
      apply rngl_leb_le in Hzs1, Hzs2, Hzs23, Hzs12.
      remember (θ3 + angle_right)%A as θ.
      ...
      assert (H : (0 < rngl_cos θ1)%L). {
        ...
        apply (eq_rngl_cos_0 Hic Hon Hop Hed) in H.
        destruct H; subst θ1. {
          rewrite (rngl_sin_sub_right_l Hon Hos) in Haov'.
          rewrite (rngl_cos_sub_right_l Hon Hop) in Hzs12.
          cbn in Haov'.
          apply (rngl_le_antisymm Hor) in Haov'. 2: {
            apply (rngl_cos_bound Hon Hop Hiv Hic Hed Hor).
          }
          apply (eq_rngl_cos_1 Hic Hon Hop Hed) in Haov'.
          subst θ2.
          rewrite (angle_add_0_l Hon Hos) in Hzs23.
          now apply (rngl_nlt_ge Hor) in Hzs23.
        }
    ...
    destruct (rngl_le_dec Hor (rngl_cos θ1) 0) as [H| H]. {
      ...
      destruct H; subst θ1. {
        ...
        apply rngl_leb_le in Haov'.
        rewrite (rngl_cos_add_right_l Hon Hop) in Haov'.
        rewrite (rngl_cos_sub_right_l Hon Hop) in Haov'.
        cbn in Haov'.
        specialize (rngl_sin_bound Hon Hop Hiv Hic Hed Hor θ2) as H1.
        apply (rngl_le_antisymm Hor) in Haov'; [ | easy ].
        symmetry in Haov'.
        apply (eq_rngl_sin_opp_1 Hic Hon Hop Hed) in Haov'.
        subst θ2.
        cbn in Hzc2.
...
...
...
      do 2 rewrite <- (angle_add_assoc Hop) in Hs23z.
      rewrite (angle_straight_add_straight Hon Hop) in Hs23z.
      rewrite (angle_add_0_r Hon Hos) in Hs23z.
      now apply (rngl_nlt_ge Hor) in Hzs23.
    }
...
      apply (rngl_nlt_ge Hor) in Hzs23.
...
...
    ...
    apply (rngl_opp_neg_pos Hop Hor) in Hzs3.
    move Hzs2 after Hzs3; move Hzs1 after Hzs2.
    rewrite angle_add_opp_r in Hzs23 |-*.
Inspect 1.
...
    cbn in Hzs23 |-*.
    ...
    apply (rngl_mul_le_mono_pos_r Hop Hor Hii _ _ (rngl_sin θ3)); [ easy | ].
    rewrite rngl_mul_add_distr_r.
    apply (rngl_mul_le_mono_pos_r Hop Hor Hii _ _ (rngl_cos θ3)) in Hzs23.
    apply (rngl_le_add_le_sub_r Hop Hor).
    rewrite (rngl_mul_mul_swap Hic).
    eapply (rngl_le_trans Hor); [ apply Hzs23 | ].
    rewrite <- (rngl_mul_assoc _ (rngl_cos θ3)).
    rewrite <- (rngl_mul_assoc _ (rngl_sin θ3)).
    do 2 rewrite fold_rngl_squ.
    ...
    apply -> (rngl_le_0_sub Hop Hor) in Hzs12.
    apply (rngl_le_0_sub Hop Hor).
    apply (rngl_mul_le_mono_pos_l Hop Hor Hii _ _ (rngl_cos θ1)); [ admit | ].
    rewrite rngl_mul_assoc.
    rewrite fold_rngl_squ.
    specialize (cos2_sin2_1 Hon Hop Hic Hed θ1) as H1.
    apply (rngl_add_move_r Hop) in H1.
...
...
  ...
  rewrite <- (rngl_mul_sub_distr_l Hop).
  apply (rngl_mul_le_mono_pos_l Hop Hor Hii _ _ (rngl_cos θ1)); [ easy | ].
  rewrite rngl_mul_assoc.
  rewrite fold_rngl_squ.
  specialize (cos2_sin2_1 Hon Hop Hic Hed θ1) as H1.
  apply (rngl_add_move_r Hop) in H1.
...
(*
...
destruct (rngl_le_dec Hor (rngl_cos θ1) (rngl_cos (θ2 + θ3))) as [Hc123| Hc231].
...
        progress unfold angle_add_overflow in Haov.
        progress unfold angle_ltb in Haov.
        rewrite (angle_add_assoc Hop) in Haov.
...
*)
        destruct (rngl_le_dec Hor (rngl_sin θ3) (rngl_sin (θ1 + θ2)))%L
          as [Hs321| Hs213]. {
          cbn in Hc123 |-*.
          rewrite (rngl_mul_opp_r Hop).
          rewrite (rngl_sub_opp_r Hop).
          apply (rngl_le_add_le_sub_r Hop Hor).
          apply (rngl_mul_le_mono_nonneg_r Hop Hor _ _ (rngl_cos θ2)) in Hc123;
            [ | easy ].
          rewrite (rngl_mul_sub_distr_r Hop) in Hc123.
          rewrite <- rngl_mul_assoc in Hc123.
          rewrite fold_rngl_squ in Hc123.
          specialize (cos2_sin2_1 Hon Hop Hic Hed θ2) as H1.
          apply (rngl_add_move_r Hop) in H1.
          ...
          rewrite (rngl_mul_sub_distr_l Hop) in Hc123.
          rewrite (rngl_mul_1_r Hon) in Hc123.
          rewrite <- (rngl_sub_add_distr Hos) in Hc123.
          progress unfold rngl_sub in Hc123 |-*.
          rewrite Hop in Hc123 |-*.
          apply (rngl_le_sub_le_add_l Hop Hor) in Hc123.
          apply (rngl_le_sub_le_add_l Hop Hor).
          progress unfold rngl_squ in Hc123.
          rewrite (rngl_mul_mul_swap Hic) in Hc123.
          rewrite rngl_mul_assoc in Hc123.
          rewrite <- rngl_mul_add_distr_r in Hc123.
          rewrite (rngl_mul_comm Hic (rngl_cos θ1)) in Hc123.
          rewrite (rngl_mul_comm Hic (rngl_sin θ1)) in Hc123.
          rewrite (fold_sin_add) in Hc123.
          eapply (rngl_le_trans Hor); [ apply Hc123 | ].
          apply -> (rngl_opp_le_compat Hop Hor).
        ...
        destruct (rngl_le_dec Hor 0 (rngl_cos θ3))%L as [Hzc3| Hc3z]. {
          ...
          destruct (rngl_eq_dec Hed (rngl_sin θ1) 0) as [Hs1z| Hs1z]. {
            apply (eq_rngl_sin_0 Hic Hon Hop Hed) in Hs1z.
...
...
          ...
          rewrite (rngl_sub_opp_r Hop).
          apply (rngl_le_add_le_sub_r Hop Hor).
          apply (rngl_lt_add_lt_sub_l Hop Hor) in Hs213.
          apply (rngl_lt_le_incl Hor) in Hs213.
          apply (rngl_mul_le_mono_nonneg_r Hop Hor _ _ (rngl_cos θ3))
            in Hs213; [ | easy ].
          ...
          rewrite rngl_mul_assoc.
...
          eapply (rngl_le_trans Hor); [ apply Hs213 | ].
          rewrite (rngl_mul_sub_distr_r Hop).
          rewrite (rngl_mul_sub_distr_l Hop).
          rewrite (rngl_mul_comm Hic (rngl_sin θ3)).
          rewrite rngl_mul_assoc.
...
...
          cbn.
          apply (rngl_le_add_le_sub_r Hop Hor).
          apply (rngl_mul_le_mono_pos_l Hop Hor Hii _ _ (rngl_sin θ1) Hc1z).
          apply (rngl_mul_le_mono_pos_r Hop Hor Hii _ _ (rngl_sin θ3) Hc3z)
            in Hc123.
          rewrite rngl_mul_assoc.
          eapply (rngl_le_trans Hor); [ apply Hc123 | ].
          rewrite (rngl_mul_add_distr_r).
...
...
          ...
          rewrite <- (rngl_mul_assoc (rngl_cos θ3)).
          rewrite <- (rngl_add_sub_assoc Hop).
          rewrite <- (rngl_mul_sub_distr_l Hop).
...
          apply (rngl_le_add_le_sub_r Hop Hor).
...
...
progress unfold angle_eucl_dist.
remember (θ1 + θ2)%A as θ.
remember (θ1 + θ3)%A as θ'.
cbn; subst θ θ'.
do 2 rewrite (rngl_sub_0_l Hop).
do 2 rewrite (rngl_squ_opp Hop).
do 2 rewrite (rngl_squ_sub Hop Hic Hon).
...
do 2 rewrite (cos2_sin2_1 Hon Hop Hic Hed).
...
specialize (cos2_sin2_1 Hon Hop Hic Hed (θ1 + θ2))%A as H1.
specialize (cos2_sin2_1 Hon Hop Hic Hed (θ1 + θ2))%A as H1.
apply (rngl_add_move_l Hop) in H1.
rewrite <- H1; clear H1.
...
rewrite (rngl_abs_nonneg_eq Hop Hor); [ | easy ].
rewrite (rngl_abs_nonneg_eq Hop Hor (1 - _))%L; [ | easy ].
do 2 rewrite <- (rngl_sub_sub_distr Hop).
apply (rngl_sub_le_mono_l Hop Hor).
...
destruct (rngl_lt_dec Hor (rngl_cos θ') 0) as [Hz'| Hz']. {
  ...
  apply -> (rngl_opp_le_compat Hop Hor).
  rename θ into x; rename θ' into θ; rename x into θ'.
  rename Hz into x; rename Hz' into Hz; rename x into Hz'.
  move Hz after Hz'.
  do 2 rewrite (rngl_add_comm (rngl_abs _)).
...
  Hz : (0 < rngl_cos θ)%L
  Hz' : (0 < rngl_cos θ')%L
  Hθθ : (rngl_cos θ ≤ rngl_cos θ')%L
  ============================
  (rngl_cos θ + rngl_abs (rngl_sin θ) ≤ rngl_cos θ' + rngl_abs (rngl_sin θ'))%L
...
...
*)
            ...
            rewrite (rngl_sin_add_right_r Hon Hos) in Hzs3, Hzs13, Hs32.
            rewrite (rngl_cos_add_right_r Hon Hop) in Hc3z, H12 |-*.
            apply (rngl_opp_neg_pos Hop Hor) in Hc3z.
            ...
            rewrite (angle_add_comm Hic angle_right).
            rewrite (rngl_cos_add_right_r Hon Hop).
            rewrite (rngl_sin_sub_right_l Hon Hos) in Hzs2, Hs32.
            apply -> (rngl_opp_le_compat Hop Hor).
            apply -> (rngl_le_0_sub Hop Hor) in H12.
            destruct (rngl_le_dec Hor (rngl_sin θ1) (rngl_sin θ2))
                as [Hs12| Hs21]. {
...
(*
...
replace θ3 with (θ1 + θ3 - θ1)%A in H23.
remember (θ1 + θ3)%A as θ eqn:Hθ.
symmetry in Hθ.
apply (angle_add_move_l Hic Hon Hop Hed) in Hθ.
subst θ3; rename θ into θ3.
cbn in H23.
cbn in H12.
...
