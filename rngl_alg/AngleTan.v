...
Theorem right_derivable_continuous :
  rngl_mul_is_comm T = true →
  rngl_has_1 T = true →
  rngl_has_inv T = true →
  ∀ A le lt, (∀ x, ¬ (lt x x)) → (∀ x y, le x y → lt x y) →
  ∀ da (f : A → T) x a,
  right_derivative_at lt da rngl_distance f x a
  → right_continuous_at le da rngl_distance f x.
Proof.
intros Hic Hon Hiv.
specialize (rngl_has_opp_has_opp_or_subt Hop) as Hos.
...
destruct b. {
  ...
  destruct c. 2: {
    apply (rngl_leb_gt Hor) in Hc.
    clear - Hc Hor rp a Hb Hop Hii Hon Hiv Hdz Hos H2 Hic H2z Hfz Hz2 Hε.
    destruct (rngl_le_dec Hor a 0) as [Hflz| Hflz]. {
      apply (rngl_nle_gt_iff Hor).
      intros Hea.
      apply rngl_nlt_ge in Hb.
...
Theorem right_derivative_mul_at :
  ...
  ∀ A lt, (∀ x, ¬ (lt x x)) →
  ∀ da (f g : A → T) u v x₀,
  right_derivative_at lt da rngl_distance f x₀ u
  → right_derivative_at lt da rngl_distance g x₀ v
  → right_derivative_at lt da rngl_distance (λ x : A, (f x * g x)%L)
       x₀ (f x₀ * v + u * g x₀)%L.
Proof.
intros Hic Hon Hiv.
specialize (rngl_has_opp_has_opp_or_subt Hop) as Hos.
...
set (K := (rngl_abs v + 1)%L).
generalize Hf; intros H11.
apply (right_derivable_continuous Hic Hon Hiv) with (le := lt) in H11;
  cycle 1. {
  apply Hlti.
} {
  easy.
...
specialize (rngl_0_le_2 Hon Hos Hor) as Hz2'.
intros ε Hε.
specialize (Hlfc (ε / (2 * rngl_abs v + 2)))%L as H1.
assert (H : (0 < ε / (2 * rngl_abs v + 2))%L). {
  apply (rngl_div_pos Hon Hop Hiv Hor); [ easy | ].
...
assert (H51 : (rngl_abs (g x) < 1 + rngl_abs v)%L). {
...
Theorem left_continuous_mul_at :
  rngl_mul_is_comm T = true →
  rngl_has_1 T = true →
  rngl_has_inv T = true →
  ∀ A le,
  ∀ da (f g : A → T) x₀,
  left_continuous_at le da rngl_distance f x₀
  → left_continuous_at le da rngl_distance g x₀
  → left_continuous_at le da rngl_distance (λ x : A, (f x * g x)%L) x₀.
Proof.
intros Hic Hon Hiv * Hlfc Hlgc.
now apply (left_or_right_continuous_mul_at Hic Hon Hiv).
Qed.

Theorem right_continuous_mul_at :
  rngl_mul_is_comm T = true →
  rngl_has_1 T = true →
  rngl_has_inv T = true →
  ∀ A le,
  ∀ da (f g : A → T) x₀,
  right_continuous_at le da rngl_distance f x₀
  → right_continuous_at le da rngl_distance g x₀
  → right_continuous_at le da rngl_distance (λ x : A, (f x * g x)%L) x₀.
Proof.
intros Hic Hon Hiv * Hlfc Hlgc.
now apply (left_or_right_continuous_mul_at Hic Hon Hiv).
Qed.
...
split; [ now apply (left_continuous_mul_at Hic Hon Hiv) | ].
split; [ now apply (right_continuous_mul_at Hic Hon Hiv) | ].
split. {
