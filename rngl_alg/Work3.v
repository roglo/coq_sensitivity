...
Search (_² + _² = 0)%L.
Search (_ * _ + _ * _ = 0)%L.
(* apparemment, il faut qu'il y ait un opp ; un subt ne suffit pas
   du moins pour ces théorèmes, mais ont-ils raison ? *)
About eq_rngl_add_square_0.
(*
rl_integral_modulus_prop:
...
eq_rngl_add_square_0:
  ∀ {T : Type} {ro : ring_like_op T},
    ring_like_prop T
    → rngl_has_opp T = true
      → rngl_is_ordered T = true
        → (rngl_is_integral_domain T || rngl_has_inv_and_1_or_quot T)%bool = true
          → ∀ a b : T, (a * a + b * b)%L = 0%L → a = 0%L ∧ b = 0%L
*)
Search (_ + _ = 0)%L.
apply rl_integral_modulus_prop in Hba.
...
Theorem glop :
