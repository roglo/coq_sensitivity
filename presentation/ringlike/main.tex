...
\frm{Reminder}{
  ...
  \begin{tabular}{lll}
    ...
    Ring \only<1-2>{?} &
    \only<3->{: & $\mathbb{Z}$
      \hsp (opposites, but no inverses) - prime numbers \\
    & & square matrices} \\
    Field \only<1-3>{?} &
    \only<4->{: & $\mathbb{Q}$, $\mathbb{R}$, $\mathbb{C}, ...$
      \hsp (opposites and inverses)} \\
  \end{tabular}

  \only<5> {
    \vsp \hsp \hsp All of them are ``ring-like''\\
    \hsp \hsp Two operations: $+$ and $*$, and a lot of axioms in common}
}
