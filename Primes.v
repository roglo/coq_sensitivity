...
assert
  ...
  apply NoDup_Permutation_bis; cycle 1. {
    now rewrite seq_length, map_length, seq_length.
  } {
    unfold incl.
    intros i Hi.
    apply in_map_iff in Hi.
    destruct Hi as (j & Hij & Hj).
    subst i.
    apply in_seq.
    apply in_seq in Hj.
    replace (1 + (p - 1)) with p in Hj |-* by flia Hpz.
    split; [ | now apply Nat.mod_upper_bound ].
    enough (H : (j * a) mod p ≠ 0) by flia H.
    intros Hja.
    apply Nat.mod_divides in Hja; [ | flia Hj ].
    destruct Hja as (k, Hk).
...
  apply NoDup_Permutation; [ | apply seq_NoDup | ]. 2: {
    intros i.
    split. {
      intros Hi.
      apply in_map_iff in Hi.
